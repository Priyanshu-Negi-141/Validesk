...
router.post("/update-instrument/:clientName/:id", async (req, res) => {
  const { clientName, id } = req.params;
  const instrumentData = req.body;

  try {
    // Find the document with the matching client name
    const srfData = await CalibrationSRFData.findOne({ "srf.client_name": clientName, "_id": id });
    ...
    const maxCounterDoc = await CalibrationSRFData.findOne({}, { "srf.mainCertificateNumber": 1 }).sort({ "srf.mainCertificateNumber": -1 }).limit(1);
    let counter = 1;
    if (maxCounterDoc && maxCounterDoc.srf.length > 0) {
      const lastCertificateNumber = maxCounterDoc.srf[0].mainCertificateNumber ?? "STC/DDN/CAL/00000";
    ...
    return res.status(200).json({ message: "Instrument data stored successfully", mainCertificateNumber });
  } catch (error) {
    console.error(error);
...
router.post("/pressureUnit-data/:instrumentName/:id", async (req, res) => {
  ...
  try {
    ...
    if (!instrumentData.srf[0].instrumentDetails[0].instrument.pressureUnitDetails) {
    ...
    CalibrationSRFData.findById(clientId)
      .then((data) => {
        if (!data) {
          ...
          return res.status(404).json({ message: "No table found for the provided client ID." });
        }
...
router.get("/instrument-data/:clientName/:id", async (req, res) => {
  try {
    ...
    const result = await CalibrationSRFData.findOne(
      { _id: id, "srf.client_name": clientName }
    );
...
router.post("/fetch-data", async (req, res) => {
  try {
    const { instrumentName, idNumber } = req.body;
    
    // Fetch the data from the database based on the instrument name and serial number
    const data = await CalibrationMasterData.findOne({
...
// submiting instrument and Table data


// POST endpoint to send data to deviceData based on instrument name and ID
router.post("/instrument/:instrumentName/:instrumentId/device", async (req, res) => {
  ...
  try {
    ...
    if (result) {
      ...
      if (instrumentDetailsIndex !== -1) {
        ...
        result.srf[0].instrumentDetails[instrumentDetailsIndex].deviceDetails.push(jsonData);
...
router.post('/store-authorized-by/:instrumentName/:id', async (req, res) => {
  ...
  try {
    ...
    await srf.save();
    return res.status(200).json({ message: 'Authorized data saved successfully' });
  } catch (error) {
...
router.get("/fetch-instru-details/:instrumentName/:id", async (req, res) => {
  ...
  // fetching calibrrated at and reviewd by data from EmployeeDetails
  
  router.get('/fetchEmployeesByCalibrationEngineer/:calibrationEngineerStatus', async (req, res) => {
    try {
      ...
      if (typeof isCalibrationEngineer !== 'boolean') {
        return res.status(400).json({ error: 'Invalid calibrationEngineerStatus value. It should be true or false.' });
      }
      ...
      if (employees.length === 0) {
        return res.status(404).json({ message: 'No employees found with the provided calibrationEngineer status.' });
      }
  ...
  router.post('/fetchEmployeesByRole', async (req, res) => {
    try {
      ...
      if (!Array.isArray(roles)) {
        return res.status(400).json({ error: 'Invalid roles. Roles should be provided as an array.' });
      }
      ...
      if (employees.length === 0) {
        return res.status(404).json({ message: 'No employees found with the provided filters.' });
      }
